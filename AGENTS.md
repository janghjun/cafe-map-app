# 카공 어디가? 인천편 - Codex 바이브코딩 운영 문서

> 이 문서는 VSCode + Codex로 `카공 어디가? 인천편`을 개발하기 위한 프로젝트 기준 문서입니다.  
> Codex는 구현 엔진이며, 이 문서는 Codex가 프로젝트의 목적, 범위, 금지사항, 구현 순서, 테스트 기준을 항상 같은 기준으로 이해하도록 돕습니다.
...
> 실제 프로젝트 스캐폴딩 이후 변경 가능하지만, Codex는 기본적으로 아래 스택을 기준으로 판단한다.
...
Codex에게 작업을 요청할 때는 반드시 아래 형식을 사용한다.
